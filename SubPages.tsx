...
export const JoinCollective: React.FC<{ setPage: (p: string) => void }> = ({ setPage }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🛡️ Sentinel: Prevent leaking database schema or error details to the UI/console
      setError('Synchronization failed. Please check your credentials or try again later.');
    } finally {
      setLoading(false);
    }
  };
