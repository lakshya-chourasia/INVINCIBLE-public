    ...
    const move = (e: MouseEvent) => {
      ...
      if (!rafId) {
        rafId = requestAnimationFrame(() => {
          ...
          // ⚡ BOLT OPTIMIZATION: Use the captured e.target rather than calling
          // document.elementFromPoint() to prevent synchronous layout recalculations
          // (layout thrashing) during this high-frequency mousemove loop.
          setIsClickable(!!eventTarget?.closest('button, a, .interactive, input, .sm-toggle, .sm-resize-handle, .sm-logo'));
          rafId = 0;
        });
      }
    };
