    ...
    const move = (e: MouseEvent) => {
      ...
      if (!rafId) {
        rafId = requestAnimationFrame(() => {
          ...
          // Use e.target from outer scope instead of synchronous document.elementFromPoint to prevent layout thrashing
          setIsClickable(!!(target?.closest && target.closest('button, a, .interactive, input, .sm-toggle, .sm-resize-handle, .sm-logo')));
          rafId = 0;
        });
      }
    };
