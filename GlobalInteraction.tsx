...
export const GlobalInteraction: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const move = (e: MouseEvent) => {
      mouseX = e.clientX;
      mouseY = e.clientY;
      hoveredTarget = e.target as HTMLElement;
      ...
      if (!rafId) {
        rafId = requestAnimationFrame(() => {
          ...
          // Use the captured e.target to avoid synchronous layout recalculation from elementFromPoint
          setIsClickable(!!hoveredTarget?.closest('button, a, .interactive, input, .sm-toggle, .sm-resize-handle, .sm-logo'));
          rafId = 0;
        });
      }
    };
