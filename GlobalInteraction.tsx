    ...
    const move = (e: MouseEvent) => {
      ...
      if (!rafId) {
        rafId = requestAnimationFrame(() => {
          ...
          // Performance: Update styles directly on the element ref instead of document.documentElement
          // This avoids invalidating styles for the entire document tree
          if (glowRef.current) {
            glowRef.current.style.setProperty('--x', `${mouseX}px`);
            glowRef.current.style.setProperty('--y', `${mouseY}px`);
          }

          // Performance: Use captured event target instead of elementFromPoint
          // elementFromPoint forces layout/reflow which is expensive during mousemove
