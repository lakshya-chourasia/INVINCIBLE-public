...
export const GlobalInteraction: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const move = (e: MouseEvent) => {
      mouseX = e.clientX;
      mouseY = e.clientY;
      targetNode = e.target as HTMLElement;
      ...
      if (!rafId) {
        rafId = requestAnimationFrame(() => {
          ...
          // Use the captured target instead of expensive document.elementFromPoint
          setIsClickable(!!targetNode?.closest('button, a, .interactive, input, .sm-toggle, .sm-resize-handle, .sm-logo'));
          rafId = 0;
        });
      }
    };
