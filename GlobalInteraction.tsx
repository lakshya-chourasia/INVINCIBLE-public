    ...
    const move = (e: MouseEvent) => {
      ...
      if (!rafId) {
        rafId = requestAnimationFrame(() => {
          ...
          // Optimization: Avoid forced synchronous layout recalculation by using e.target
          // instead of document.elementFromPoint
          setIsClickable(!!hoveredElement?.closest?.('button, a, .interactive, input, .sm-toggle, .sm-resize-handle, .sm-logo'));
          rafId = 0;
        });
      }
    };
